...
- **回测/评分优化**：当用户要求验证评分算法、历史区间、TopN 收益、显著性或 HTML 可视化时，按 [backtesting.md](references/backtesting.md) 使用 `scripts/alpha_backtest.js` 和 `scripts/lib/backtest_engine.js` 做 point-in-time 回测、参数网格比较、留出区间验证和产物归档。
